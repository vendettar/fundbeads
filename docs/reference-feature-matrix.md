...
3. Advanced manual editing tools such as flood fill, magic wand, lasso, rectangular selection, multi-select, layers, and edit persistence should remain separate feature instructions.
4. Future MARD editions, palette filtering, fixed-dimension crop/viewport controls, and mobile grid navigation are useful follow-ups but not current MVP scope.
