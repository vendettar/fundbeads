...
| Processing status | Upload and reprocessing work tracks busy state internally without inserting visible processing text that shifts the layout. | `App`, `UploadWorkspace`, `frontend/src/use-pattern-processing.ts` | Progress percentages and visible processing banners are not implemented. |
...
| Image pixelation | Canvas draws the full source image into aspect-ratio-derived output dimensions. Worker-backed processing can be aborted when superseded. | `imageFileToPattern`, `usePatternProcessing` | Fixed-size crop/drag framing is not implemented. |
...
| Pattern grid | DOM grid with colored cells and MARD codes. Defaults to fit-to-screen. Row render models are reused so a one-cell edit in a `100x100` grid only rebuilds the affected row model. | `frontend/src/pattern-grid.tsx`, `frontend/src/pattern-grid-board.tsx`, `frontend/src/pattern-render-model.ts` | Real-browser visual/performance checks remain manual while E2E is skipped. |
...
| Pattern export | Browser-local PNG and PDF downloads render from the effective `Pattern` and current preview toggles for grid, MARD codes, and row/column labels. PDF export uses a single-page image layout and shares render geometry/model helpers with the DOM preview. | `frontend/src/pattern-export.ts`, `frontend/src/pattern-render-model.ts`, `PatternGrid` | Paginated assembly booklets, export metadata panels, and color-summary pages are not implemented. |
...
| Design-rule guardrails | Layering uses named CSS layer classes, global CSS is limited to approved files/categories, static arbitrary JSX utilities are blocked, and native controls follow approved accessibility patterns. | `docs/design-rules.md`, `frontend/test/source-guard.test.ts`, `frontend/test/i18n-theme.test.ts` | New overlays, global CSS files, static bracket utilities, or custom controls require rule updates and tests. |
| Preferences | Language, theme, and interface style stored only in browser `localStorage` when available. Preference menus clamp/flip within the viewport. | `frontend/src/browser-storage.ts`, `frontend/src/preference-select.tsx` | Must remain optional and safe when storage is blocked. |
| Local pattern persistence infrastructure | IndexedDB module for compact, validated local pattern records, including no-bead cells as `null` row-major `cellCodes`. No history, draft, offline library, source-image save UI, or account sync UI is shipped yet. | `frontend/src/local-pattern-db.ts` | Future persistence UI must preserve local-only privacy, schema validation, pruning, and account-sync boundaries. |
...
- `100x100` produces 10,000 bead cells plus axes. Unit coverage enforces bounded row-model churn, but real-browser visual/performance checks are still manual because E2E is currently skipped.
...
- Static layout styles can drift if developers bypass named CSS primitives with arbitrary JSX utilities or raw z-index values.
